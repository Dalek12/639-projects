...
def gram_matrix(features, normalize=True):
    ...
    gram = torch.matmul(features_reshaped, features_reshaped.transpose(1, 2))
    if normalize:
        gram /= features.shape[1:].numel()
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
    return gram
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, layer_idx in enumerate(style_layers):
        gram_matrix_current = gram_matrix(feats[layer_idx])
        loss += style_weights[i] * torch.sum((gram_matrix_current - style_targets[i])**2)
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def tv_loss(img, tv_weight):
    ...
    loss = tv_weight * (torch.sum(torch.abs(h_diff)**2) + torch.sum(torch.abs(w_diff)**2))
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def guided_gram_matrix(features, masks, normalize=True):
  ...
  for i in range(R):
    masked_features = features[:, i] * masks[:, i].unsqueeze(1)
    reshaped_features = masked_features.view(N, C, -1)
    gram = torch.matmul(reshaped_features, reshaped_features.transpose(1, 2))
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    for i, layer in enumerate(style_layers):
        ...
        layer_masks = content_masks[layer]
        layer_targets = style_targets[i]
        layer_weight = style_weights[i]

        N, R, C, H, W = layer_feats.shape
        layer_feats = layer_feats.view(N, R, C, H, W)
        layer_masks = layer_masks.view(N, R, H, W)

        layer_guided_gram = guided_gram_matrix(layer_feats, layer_masks)
        layer_loss = torch.sum((layer_guided_gram - layer_targets) ** 2)
