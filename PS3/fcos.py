...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        for i, level_name in enumerate(["c3", "c4", "c5"]):
          in_channels = dummy_out_shapes[i][1][1]

          # Lateral 1x1 convolution
          self.fpn_params[f"lateral_{level_name}"] = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1)

          # Output 3x3 convolution
          if level_name == "c3":
              # (Conv, BN, ReLU, Conv) structure for p3
              self.fpn_params[f"output_p3"] = nn.Sequential(
              ...
              # Single 3x3 conv layer for p4 and p5
              self.fpn_params[f"output_p{4 if level_name == 'c4' else 5}"] = nn.Conv2d(
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        ######################################################################
        # Replace "PASS" statement with your code
        y_coords = torch.arange(0, feat_shape[2] * level_stride, \
        step=level_stride, dtype=torch.float32, device=device)+ level_stride / 2
        x_coords = torch.arange(0, feat_shape[3] * level_stride, \
        step=level_stride, dtype=torch.float32, device=device)+ level_stride / 2
        y_coords, x_coords = torch.meshgrid(y_coords,x_coords)
        
        # Stack the coordinates and reshape the tensor
        level_coords = torch.stack((y_coords,x_coords), dim=2).view(-1, 2)
        location_coords[level_name] = level_coords
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    while sorted_indices.numel() > 0:
        ...
        tl = torch.max(remaining_boxes[:, :2], current_box[:2])  # top-left coordinates
        br = torch.min(remaining_boxes[:, 2:], current_box[2:])  # bottom-right coordinates
        wh = (br - tl).clamp(min=0)  # width-height
        inter_area = wh[:, 0] * wh[:, 1]  # intersection area

        box_area = (remaining_boxes[:, 2] - remaining_boxes[:, 0]) * (remaining_boxes[:, 3] - remaining_boxes[:, 1])
        current_box_area = (current_box[2] - current_box[0]) * (current_box[3] - current_box[1])
        union_area = box_area + current_box_area - inter_area
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for i, out_channels in enumerate(stem_channels):
            stem_cls.append(nn.Conv2d(in_channels, out_channels, \
            kernel_size=3, stride=1, padding=1))
            stem_cls.append(nn.ReLU(inplace=True))
            stem_box.append(nn.Conv2d(in_channels, out_channels, \
            kernel_size=3, stride=1, padding=1))
        ...
        self.pred_cls = nn.Conv2d(stem_channels[-1], num_classes, \
        kernel_size=3, stride=1, padding=1)
        self.pred_box = nn.Conv2d(stem_channels[-1], 4, \
        kernel_size=3, stride=1, padding=1)
        self.pred_ctr = nn.Conv2d(stem_channels[-1], 1, \
        kernel_size=3, stride=1, padding=1)

        # Replace "PASS" statement with your code
        for m in [self.pred_cls, self.pred_box, self.pred_ctr]:
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feat in feats_per_fpn_level.items():
            # Obtain the outputs of stem_cls and stem_box
            stem_cls_out = self.stem_cls(feat)
            stem_box_out = self.stem_box(feat)

            # Obtain predictions for each output type
            cls_logit = self.pred_cls(stem_cls_out)
            box_delta = self.pred_box(stem_box_out)
            ctr_logit = self.pred_ctr(stem_box_out)

            # Rearrange dimensions and flatten H, W
            cls_logit = cls_logit.permute(0, 3, 2, 1).contiguous().\
            view(feat.size(0), -1, self.pred_cls.out_channels)
            box_delta = box_delta.permute(0, 3, 2, 1).contiguous().\
            view(feat.size(0), -1, 4)
            ctr_logit = ctr_logit.permute(0, 3, 2, 1).contiguous().\
            view(feat.size(0), -1, 1)
            #print('ctr_logit shape', ctr_logit.shape)
            # Store the predictions in the corresponding dictionaries
...
def fcos_get_deltas_from_locations(
    ...
    deltas = torch.stack([x1, y1, x2, y2], dim=-1) / stride
    background_mask = (gt_boxes[:, 4] == -1).unsqueeze(-1).expand_as(deltas)
...
def fcos_apply_deltas_to_locations(
    ...
    # Un-normalize deltas using stride
    deltas = deltas * stride

    # Clip deltas to be non-negative
    deltas = torch.clamp(deltas, min=0)

    # Separate the x and y coordinates from the locations tensor
    x = locations[:, 0].view(-1, 1)
    y = locations[:, 1].view(-1, 1)

    # Apply LTRB deltas to locations
    x1 = x - deltas[:, 0:1]
    y1 = y - deltas[:, 1:2]
    x2 = x + deltas[:, 2:3]
    y2 = y + deltas[:, 3:4]

    output_boxes = torch.cat([x1, y1, x2, y2], dim=-1)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    min_deltas = torch.min(left_right, dim=1)[0] * \
    torch.min(top_bottom, dim=1)[0]
    max_deltas = torch.max(left_right, dim=1)[0] * \
    torch.max(top_bottom, dim=1)[0]
...
class FCOS(nn.Module):
    ...
    def __init__(
        ...
        self.pred_net = FCOSPredictionNetwork(num_classes, fpn_channels, \
        stem_channels)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(
        ...
        # Replace "PASS" statement with your code
        fpn_out = self.backbone(images)
        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = \
        self.pred_net(fpn_out)
        ######################################################################
        ...
        # Replace "pass" statement with your code
        fpn_feats_shapes = {
          level_name: feat.shape for level_name, feat in fpn_out.items()
        }
        locations_per_fpn_level = get_fpn_location_coords(fpn_feats_shapes, \
        self.backbone.fpn_strides, device = 'cuda')
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        for gt_boxes_per_image in gt_boxes:
            matched_gt_boxes_per_image = fcos_match_locations_to_gt(
        ...
        for i in range(len(matched_gt_boxes)):
            matched_gt_deltas_per_image = {
                lvl: fcos_get_deltas_from_locations(
                    locations_per_fpn_level[lvl], matched_gt_boxes[i][lvl], \
                    stride=strides_per_fpn_level[lvl]
                )
                for lvl in ["p3", "p4", "p5"]
            }
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        #def print_min_max(tensor, name):
        #    min_val = torch.min(tensor)
        #    max_val = torch.max(tensor)
        #    print(f"{name} min: {min_val.item()}, max: {max_val.item()}")
        loss_cls, loss_box, loss_ctr = None, None, None
        # Replace "PASS" statement with your code
        dummy_gt_classes = []
        for b in range(len(gt_boxes)):
            gt_classes = matched_gt_boxes[b][:, -1].long()
            num_locations = gt_classes.shape[0]
            one_hot_gt_classes = torch.zeros(num_locations, self.num_classes, device=images.device)
            valid_gt_mask = gt_classes >= 0
            one_hot_gt_classes[valid_gt_mask] = F.one_hot(gt_classes[valid_gt_mask], self.num_classes).float()
            one_hot_gt_classes[~valid_gt_mask] = 0  # Background positions
            dummy_gt_classes.append(one_hot_gt_classes)

        # Stack tensors along the batch dimension
        dummy_gt_classes = torch.stack(dummy_gt_classes, dim=0)
        #print_min_max(pred_cls_logits, "pred_cls_logits")
        #print_min_max(dummy_gt_classes, "dummy_gt_classes")
        #print("pred_cls_logits shape: ", dummy_gt_classes.shape)
        loss_cls = sigmoid_focal_loss(
            pred_cls_logits, dummy_gt_classes, reduction="none"
        )
        #print_min_max(pred_boxreg_deltas, "pred_boxreg_deltas")
        #print_min_max(matched_gt_deltas, "matched_gt_deltas")
        ...
        gt_centerness = []
        for b in range(len(gt_boxes)):
          gt_deltas = matched_gt_deltas[b]
          centerness_targets = fcos_make_centerness_targets(gt_deltas)
          gt_centerness.append(centerness_targets)

        # Stack tensors along the batch dimension
        gt_centerness = torch.stack(gt_centerness, dim=0)
        #print_min_max(pred_ctr_logits, "pred_ctr_logits")
        #print_min_max(gt_centerness, "gt_centerness")
        loss_ctr = F.binary_cross_entropy_with_logits(
            pred_ctr_logits.squeeze(), gt_centerness, reduction="none"
        )
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "PASS" statement with your code
            max_scores, max_classes = torch.max(level_pred_scores, dim=1)
            # Step 2:
            ...
            level_locations, stride = 8)

            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "PASS" statement with your code
            height, width = images.shape[-2], images.shape[-1]
            level_pred_boxes[:, 0::2].clamp_(min=0, max=width - 1)
            level_pred_boxes[:, 1::2].clamp_(min=0, max=height - 1)
            #level_pred_boxes = level_pred_boxes.view(-1, 4)
            #level_pred_classes = level_pred_classes.view(-1)
            #level_pred_scores = level_pred_scores.view(-1)
            #print("level_pred_scores shape:", level_pred_scores.shape)
            #print("level_pred_scores:", level_pred_scores)
